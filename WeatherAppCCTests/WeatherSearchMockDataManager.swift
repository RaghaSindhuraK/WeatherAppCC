//
//  WeatherSearchMockDataManager.swift
//  WeatherAppCCTests
//
...
class WeatherSearchMockDataManager: WeatherSearchDataManager {
    
    var didAPISucceed = true
    
    override func getWeatherDetails(with cityName: String, callback: @escaping (WeatherSearchDataModel?) -> Void) {
        if didAPISucceed {
            callback(WeatherSearchDataModel(
    ...
    override func getWeatherForCurrentLocation(callback: @escaping (WeatherSearchDataModel?) -> Void) {
        if didAPISucceed {
            callback(WeatherSearchDataModel(
                coord: WeatherCoordModel(lon: -74.006, lat: 40.7143),
                weather: [WeatherCondition(id: 801, main: "Clouds", description: "few clouds", icon: "02n")],
    ...
    override func getlonlatDetails(with lon: Double, lat: Double, callback: @escaping (WeatherSearchDataModel?) -> Void) {
        if didAPISucceed {
            callback(WeatherSearchDataModel(
                coord: WeatherCoordModel(lon: -74.006, lat: 40.7143),
                weather: [WeatherCondition(id: 801, main: "Clouds", description: "few clouds", icon: "02n")],
